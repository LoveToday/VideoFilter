...
//        [self configTextWithImage:[UIImage imageNamed:@"7.jpg"]];
//    });
...
//            [images addObject:resultImage];
    ...
    //CMTime requestTime = CMTimeMakeWithSeconds(videoSecond, timeScale);
//    NSError *error = nil;
//    CGImageRef imageRef = [gen copyCGImageAtTime:requestTime actualTime:NULL error:&error];
//
//    if (error) {
//        return nil;
//    }
...
- (void)filterInit {
    ...
    //读取图片
    UIImage *image = [UIImage imageNamed:@"test00"];
    
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"long" ofType:@"mp4"];
    AVAsset *videoAsset = [AVURLAsset assetWithURL:[NSURL fileURLWithPath:filePath]];
    
    [self getImageWithAsset:videoAsset];
